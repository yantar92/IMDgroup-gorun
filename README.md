...
-   **`--dir DIR...`:** Run the pipeline in each `DIR` instead of the
    ...
    exiting non-zero at the end if any directory failed.


## `gorun vasp`

Submit a VASP job from the current directory.  The plain `gorun`
command (without subcommand) is treated as `gorun vasp`.  `--nodes`
and `--time-limit` specify the number of nodes and the time limit:

    # Submit job requesting 2 nodes for 24 hours
    gorun vasp --nodes 2 --time-limit 24:00:00
    # Shorthand:
    gorun --nodes 2 --time-limit 24:00:00
    
    # Use default nodes/time from config (flags optional)
    gorun vasp
...
-   **`--nodes N`:** Number of nodes to request (optional; defaults come
    from config).

-   **`--time-limit HH:MM:SS`:** Time limit (optional; defaults come
    from config).
...
### Examples
    ...
    gorun vasp --queue plgrid-gpu-a100 --nodes 8 --time-limit 48:00:00
    
    # Use gamma-point-only VASP binary, 1 node, 1 hour
    gorun vasp --vasp gam --nodes 1 --time-limit 1:00:00
    
    # Run locally (no sbatch) for quick testing
    gorun vasp --local --nodes 1 --time-limit 0:30:00
    
    # Prepare directory for later batch submission
    gorun vasp --mark --nodes 4 --time-limit 24:00:00
    
    # Override INCAR settings before submission
    gorun vasp --incar ISIF:3 NSW:100 --nodes 2 --time-limit 24:00:00
    
    # Force re-run even if already converged
    gorun vasp --force --nodes 2 --time-limit 24:00:00
    
    # Limit concurrent jobs to 20
    gorun vasp --max-slurm-jobs 20 --nodes 2 --time-limit 24:00:00
...
### Examples
    ...
    gorun mace-finetune --time-limit 48:00:00
...
### Examples
    ...
    gorun gpu --time-limit 48:00:00
    
    # Use a custom script
    gorun gpu --script-name train.py --time-limit 24:00:00
    
    # Run locally for testing
    gorun gpu --local
    
    # Prepare directory for later batch submission
    gorun gpu --mark --time-limit 48:00:00
    
    # Submit to a specific queue
    gorun gpu --queue plgrid-gpu-a100 --time-limit 24:00:00
...
-   **`--nodes N`:** Number of nodes for the `maps` parent job
    (default: `1`).

-   **`--time-limit HH:MM:SS`:** Time limit for the `maps` parent job
    (optional).
